...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  const colorTransitionRef = useRef<{ start: number; from: number[]; to: number[] } | null>(null);
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (colorTransition) {
        const progress = Math.min(1, (t - colorTransition.start) / 720);
        const eased = progress * progress * (3 - 2 * progress);
        const uniforms = program.uniforms as Record<string, { value: Float32Array }>;
        const colors = [uniforms.uHorizonColor.value, uniforms.uWaveColor.value, uniforms.uCrestColor.value];
        colors.forEach((color, colorIndex) => {
          for (let channel = 0; channel < 3; channel++) {
            const offset = colorIndex * 3 + channel;
            color[channel] = colorTransition.from[offset] + (colorTransition.to[offset] - colorTransition.from[offset]) * eased;
          }
        });
        if (progress >= 1) colorTransitionRef.current = null;
      }
  ...
  useEffect(() => {
    ...
    if (!colorInitializedRef.current) {
      [hc, wc, cc].forEach((color, index) => color.set(target.slice(index * 3, index * 3 + 3)));
  ...
  return <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }} className={className} />;
};
