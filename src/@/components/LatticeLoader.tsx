...
export type LatticePatternName =
  ...
  | 'rain'
  | 'pulse';
export type LatticeGrid = 3 | 4 | 5 | 10;
...
const PATTERNS: Record<LatticePatternName, Partial<Record<LatticeGrid, ResolvedPattern>>> = {
  ...
  sweep: {
    ...
    5: { cells: Array.from({ length: 25 }, (_, i) => Math.floor(i / 5) + (i % 5)), loop: 10, scale: 1, lit: 0.45 },
    10: { cells: Array.from({ length: 100 }, (_, i) => Math.floor(i / 10) + (i % 10)), loop: 20, scale: 1, lit: 0.45 }
  },
...
const DEFAULT_PATTERN: Record<LatticeGrid, LatticePatternName> = { 3: 'orbit', 4: 'sweep', 5: 'sweep', 10: 'sweep' };
...
const LatticeLoader: React.FC<LatticeLoaderProps> = ({
  ...
  const n: LatticeGrid = grid === 10 ? 10 : grid === 5 ? 5 : grid === 4 ? 4 : 3;
