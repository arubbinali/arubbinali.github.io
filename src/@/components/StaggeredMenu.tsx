...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const wasClosing = closeTweenRef.current !== null;
  ...
  const playClose = useCallback(() => {
    ...
    // Match the individual slide durations used when opening. The opening
    // timeline is longer only because the links continue animating afterward.
    if (itemEls.length) {
      tl.to(itemEls, {
        yPercent: 140,
        rotate: 10,
    ...
    if (socialTitle) tl.to(socialTitle, { opacity: 0, duration: 0.35, ease: 'power2.in' }, 0);
    if (socialLinks.length) {
      tl.to(socialLinks, {
        y: 25,
        opacity: 0,
    ...
    tl.to(panel, { xPercent: offscreen, duration: 0.75, ease: 'power2.inOut' }, 0);
